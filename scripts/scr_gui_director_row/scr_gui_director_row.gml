/**
 * @desc GUIDirector that implements a row layout
 */
function GUIDirectorRow(): GUIDirector() constructor {
	...
	static update_content_width = function(_gui) {
		...
		_content_width = array_reduce(_gui.children, function(_prev, _child) {
			...
			return _prev + _child.margin.left + _child.margin.right + _child.width;
		}, _content_width);
	...
	static update_content_height = function(_gui) {
		...
		_content_height = array_reduce(_gui.children, function(_prev, _child) {
			...
			return max(_prev, _child.margin.top + _child.margin.bottom + _child.height);
		}, _content_height);
	...
	static update_children_x_position = function(_gui) {
		...
		for(var _i = 0; _i < _children_amount; _i++) {
			...
			if(_last_sibling_in_flow_index != -1) {
				...
				_child.x = _last_sibling_in_flow.x + _last_sibling_in_flow.width + _last_sibling_in_flow.margin.right + _child.margin.left;
			}
